...
router.post('/users', async (req, res) => {
  try {
    ...
    // Now create activation key automatically (online model)
    ...
    // Generate activation key via service
    const result = await activationKeyService.generateKey({
      fullName,
      email: email.toLowerCase(),
      role,
      facility,
    ...
    if (!result.success || !result.data) {
      ...
      return res.status(500).json({
        success: false,
        error: result.error || 'Failed to generate activation key',
    ...
    user.activationKeyExpires = expiresAt;
    await user.save();
